### Thermus thermophilus protein secondary structure HMM (3 states)
...
data.full <- data.frame(read.csv2('E:/Project/Bioinformatics project/ss-filtered-3.csv'))
...
runHMM <- function(data, testfold)
{
  ...
  i.matrix <- SS.HMM$A[1, 2:4]
  t.matrix <- SS.HMM$A[2:4, 2:4]
  e.matrix <- SS.HMM$E
  hmm <- initHMM(s.structures, a.acids, i.matrix, matrix(c(t.matrix), 3), matrix(c(e.matrix), 3))
